...
pub fn setup_semiconductor_physics(
...
) -> FunctionalPhysics<PnJunctionParams, impl FluxFn<PnJunctionParams>, impl ReactionFn<PnJunctionParams>, impl StorageFn<PnJunctionParams>> {
    fn reaction(f: &mut [DualDVec64], u: &[DualDVec64], cell: &Cell, data: &PnJunctionParams) {
    ...
    fn flux(f: &mut [DualDVec64], u_k: &[DualDVec64], u_l: &[DualDVec64], _face: &Face, data: &PnJunctionParams) {
    ...
    fn storage(f: &mut [DualDVec64], u: &[DualDVec64], _cell: &Cell, data: &PnJunctionParams) {
        let (psi, phi_n, phi_p) = (&u[0], &u[1], &u[2]);
    ...
    let physics = FunctionalPhysics::new(fields, params, flux, reaction, storage);
    physics
}

...
use vfvm_rs::physics::bc::{BCRegistry, Field};
use vfvm_rs::physics::functional::FunctionalPhysics;
use vfvm_rs::physics::functional::{FluxFn, ReactionFn, StorageFn};
use vfvm_rs::system::{Geometry, InitialCondition, OutputConfig, SolverConfig, System};
...
fn setup_electrostatics(
    params: CapacitorParams,
) -> FunctionalPhysics<
    CapacitorParams,
    impl FluxFn<CapacitorParams>,
    impl ReactionFn<CapacitorParams>,
    impl StorageFn<CapacitorParams>,
> {
    // −ε∇ϕ
    fn flux(
        f: &mut [DualDVec64],
        ...
        f[0] = (u_l[0].clone() - u_k[0].clone()) * data.epsilon;
    }

    fn reaction(f: &mut [DualDVec64], u: &[DualDVec64], cell: &Cell, data: &CapacitorParams) {
    ...
    fn storage(f: &mut [DualDVec64], u: &[DualDVec64], _cell: &Cell, _data: &CapacitorParams) {
        f[0] = u[0].clone();
    }
